...
import com.tommunyiri.dvtweatherapp.QUERY_LOCATION
...
class WeatherRepositoryTest {
    ...
    fun `assert that getSearchWeather fetches successfully from the remote source`() =
        mainCoroutineRule.runBlockingTest {
            Mockito.`when`(remoteDataSource.getSearchWeather(QUERY_LOCATION))
                .thenReturn(Result.Success(fakeNetworkWeather))

            val response = systemUnderTest.getSearchWeather(QUERY_LOCATION)

            Mockito.verify(remoteDataSource, Mockito.times(1)).getSearchWeather(QUERY_LOCATION)
